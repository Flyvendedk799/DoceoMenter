/**
 * One account's Gemini subscription, stored and kept fresh.
 *
 * Mirrors `ai-auth`'s `ClaudeAccountStore` (same `SecretBox` sealing, same store, same
 * refresh-and-write-back shape) because there is no Gemini equivalent to import — see
 * `geminiOAuth.ts`'s header for why. Two differences from Claude's version:
 *
...
const SECRET_LABEL = "doceomenter-gemini-oauth";
...
export class GeminiAccountStore {
  ...
  private async resolveToken(accountId: string): Promise<string> {
    const record = await this.options.store.read(this.key(accountId));
    if (!record) {
      throw new Error("This account has no Gemini subscription connected.");
    }

    const payload = this.box.openJson<StoredPayload>(record.payload);
    if (!payload || typeof payload.accessToken !== "string") {
      throw new Error("The stored Gemini credential could not be read. Connect the subscription again.");
    }
    ...
    if (!payload.refreshToken) {
      throw new Error("The Gemini subscription has expired and cannot be refreshed. Connect it again.");
    }
