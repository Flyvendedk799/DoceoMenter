/**
 * Signing in to a Gemini subscription from a browser, via Antigravity CLI's own OAuth client.
 *
 * There is no `ai-auth` equivalent of this file to import — the mechanism it wraps was built
 * for Antigravity CLI (`agy`), Google's replacement for the free tier of Gemini CLI, which
 * Google killed outright on 2026-06-18 (`agy`'s own client says so: sign in with the *old*
 * gemini-cli client and it answers "This client is no longer supported for Gemini Code Assist
 * for individuals. To continue using Gemini, please migrate to the Antigravity suite of
 ...
 * binary, the same way `ai-auth` got Claude Code's and the old Gemini CLI's constants. One
 * difference from both of those worth being clear-eyed about: Anthropic's Claude Code client
 * is a public PKCE client with no secret at all, and Google *published* the old Gemini CLI's
...
export function startGeminiLogin(): GeminiLoginStart {
  ...
  return { url: `${GEMINI_OAUTH.authorizeUrl}?${params.toString()}`, verifier, state };
}
...
export interface GeminiOAuthIdentity {
  accessToken: string;
 ...
 */
export async function exchangeGeminiCode(input: {
  code: string;
  verifier: string;
  fetchImpl?: typeof fetch;
  now?: () => number;
}): Promise<GeminiOAuthIdentity> {
  ...
  try {
    response = await doFetch(GEMINI_OAUTH.tokenUrl, {
      method: "POST",
      headers: { "content-type": "application/x-www-form-urlencoded" },
      body: body.toString(),
    });
  } catch (error) {
    throw new GeminiLoginError(`Could not reach Google: ${(error as Error).message}`, false);
  }
  ...
  if (typeof accessToken !== "string" || accessToken.length === 0) {
    throw new GeminiLoginError("Google returned no access token.", true);
  }
...
export async function refreshGeminiToken(
  ...
  const accessToken = json?.access_token;
  if (typeof accessToken !== "string" || accessToken.length === 0) {
    throw new GeminiLoginError("Google returned no access token on refresh.", true);
  }
  const expiresIn = typeof json?.expires_in === "number" && json.expires_in > 0 ? json.expires_in : 3600;
