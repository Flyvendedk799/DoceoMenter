...
8. **Request the `aicode` OAuth scope** (real `agy` 1.2.7 always does). Omitting it was a wrong fix — with `aicode`, personal accounts get a working consumer session on **daily** Cloud Code even when `loadCodeAssist` names `aicode-consumers`. Existing panel connects must Disconnect + Connect again so the new scopes are granted.
9. **Body-only companion fallback**: send `aicode-consumers` as generateContent `project` **without** `x-goog-user-project` (the header is what triggered serviceUsageConsumer 403). Verified: body-only and no-project both return 200 on daily with an `agy`-shaped token; header → 403.
10. **`agy` never asks for a GCP project** — after Google login it calls `loadCodeAssist` and generateContent on **daily**. DoceoMenter must match that. The panel’s **PERSONAL GCP PROJECT** field is an optional last-resort escape hatch, not the normal Connect flow.
11. **Bare model ids** on Cloud Code generateContent — `models/gemini-…` 404s. UI/`gemini-3.1-pro` maps to `gemini-3.1-pro-low` (daily’s tiered id). Prod host returns a misleading `429 RESOURCE_EXHAUSTED` for the same personal token that succeeds on daily.
12. **Prod Antigravity OAuth client** (same as `agy`), not the G1 dogfood client, for panel Connect.
